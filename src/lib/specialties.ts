// Labels used to live here alongside the styling, but that meant the
// display text could only ever be Uzbek. The id (this object's key) is
// still the single source of truth used everywhere for filtering/matching
// -- only the human-readable label moved, into the `specialties`
// translation namespace, keyed by this same id.
export const SPECIALTY_CONFIG: {
  [key: string]: { activeClass: string; inactiveClass: string; icon: string };
} = {
  All: {

...
import { Compass, Menu, X, CalendarCheck, HelpCircle, UserCheck } from 'lucide-react';
    ...
    <header className="sticky top-0 z-50 border-b border-amber-900/10 bg-[#FAF6EE]/95 backdrop-blur-md px-6 py-4">
      <div className="max-w-6xl mx-auto flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-3 group">
          <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-amber-700 via-orange-800 to-amber-950 flex items-center justify-center text-amber-100 shadow-md shadow-amber-900/20 text-xl font-serif font-black">
            🐪
          </div>
          <div>
        ...
        <nav className="hidden md:flex items-center gap-6 text-xs font-semibold text-stone-700">
          ...
          <a href="#how-it-works" className="hover:text-amber-900 transition-colors flex items-center gap-1">
            <HelpCircle className="w-3.5 h-3.5 text-amber-800" />
            Qanday ishlaydi?
          </a>
          <Link href="/my-bookings" className="hover:text-amber-900 transition-colors flex items-center gap-1 bg-amber-100/80 text-amber-950 px-3 py-1.5 rounded-full border border-amber-300/60">
      ...
      {isOpen && (
        <div className="md:hidden pt-4 pb-3 border-t border-amber-900/10 mt-3 space-y-3">
          <Link
            ...
            className="block text-sm font-semibold text-stone-800 px-2 py-1.5 hover:bg-amber-100/50 rounded-lg"
          >
            Rahnamolar katalogi
          </Link>
          <a
          ...
          <Link
            href="/my-bookings"
            onClick={() => setIsOpen(false)}
            className="block text-sm font-semibold text-amber-950 px-2 py-1.5 bg-amber-100/80 rounded-lg border border-amber-300/60"
          >
            🗓️ Mening qabullarim
          </Link>
          <a
